...
import gym
import time
import spinup.algos.pytorch.pit.epit.core as core
...
def epit(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(),  seed=0, 
    ...
    non-goal state s,
      ...
      E_{\pi'} [ R_{t+1} + \gamma v_\pi(S_{t+1}) | S_t=s ] >= v_\pi(s).

    Then, v_{\pi'}(s) >= v_\pi(s) for all states s.

    
    Motivated by the PIT, for a parametrized policy \pi_\\theta, define
    the loss L(\\theta) as

      \\frac{ \sum_{s\in S_N} E_\\theta[ R_{t+1} + \gamma v_\pi(S_{t+1}) | S_t=s ] - v_\pi(s) }{ |S_N| }

    where S_N is the set of non-goal states. Taking derivatives,
    \\nabla_theta L(\\theta) becomes

      \\frac{ \sum_{s\in S_N} E_\\theta[ q_\pi(S_t,A_t) \\nabla \ln \pi_theta(A_t|S_t) | S_t=s ] }{ |S_N| }
    ...
    *****
    ...
    def update():
        ...
        pi_l_old, pi_info_old = compute_loss_pi(data, debug=True)
        pi_l_old = pi_l_old.item()
        v_l_old = compute_loss_v(data, debug=True).item()

        # Train policy with a single step of gradient descent
        pi_optimizer.zero_grad()
        loss_pi, pi_info = compute_loss_pi(data, debug=False) # CHECK: Needed?
        loss_pi.backward()
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            #logger.log(f'Step: t={t}, a={a}, v={v}, logp={logp}, next_o={next_o}, r={r}, d={d}')
