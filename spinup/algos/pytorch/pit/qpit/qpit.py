...
def qpit(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def compute_loss_q(data, debug: int = 0):
        ...
        q = ac.q(o, a)
        if debug > 1: logger.log(f'compute_loss_q: q={q}, shape={q.shape}')

        # Bellman backup for Q function
        with torch.no_grad():
            # Q(s,a) = r(s,a) + \gamma \sum_{s'} p(s'|s,a) [ \sum_{a'} \pi(a'|s') Q(s',a') ]
            a2, _ = ac_targ.pi(o2)
            a2 = a2.sample().unsqueeze(dim=-1)
            q_pi_targ = ac_targ.q(o2, a2)
            backup = r + gamma * (1 - d) * q_pi_targ
        if debug > 1: logger.log(f'compute_loss_q: backup={backup}, shape={backup.shape}')
    ...
    def compute_loss_pi(data, debug: int = 0):
        ...
        q_pi = ac.q(o, a)
        if debug > 1: logger.log(f'compute_loss_pi: o={o}, a={a}, q_pi={q_pi}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='qpit')
    args = parser.parse_args()

    from spinup.utils.run_utils import setup_logger_kwargs
    logger_kwargs = setup_logger_kwargs(args.exp_name, args.seed)

    qpit(lambda : gym.make(args.env), actor_critic=core.MLPActorCritic,
